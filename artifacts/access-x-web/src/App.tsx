  ...
  return (
    <div className="mx-auto max-w-[1280px] px-5 py-8 pb-28 sm:px-8 sm:py-10 lg:px-10 lg:pb-12">
      <PageIntro
        ...
        title="Orient before you move."
        description="Dynamic spatial map and clock-face relative context for your route. Use alongside live audio readouts."
        action={
          <button
            type="button"
            data-testid="button-center-on-location"
            onClick={() => void environment.refetch()}
            className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-3.5 py-2.5 text-xs font-semibold hover:border-primary/50"
          >
            <LocateFixed size={15} /> Center on current read
          </button>
        }
      />
      <RouteMapVisualizer dashboard={dashboard} onCenter={() => void environment.refetch()} />
    </div>
  );
}
...
function App() {
  ...
  useEffect(() => {
    const mode = preferences.data?.accessibilityMode;
    if (mode === 'high-contrast') {
      document.documentElement.classList.add('high-contrast');
    } else {
      document.documentElement.classList.remove('high-contrast');
    }
  }, [preferences.data?.accessibilityMode]);
